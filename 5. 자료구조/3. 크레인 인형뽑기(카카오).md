    		...
    		for (int pos : moves) {
    			...
    			for(int i=0;i<board.length;i++) {
    				...
    				if(board[i][pos-1]!=0) {
    					...
    					//인형을 뽑은 자리에는 0으로 변경
    					board[i][pos-1]=0;
    					//stack이 비지 않았고 제일 위에거를 뺐을 때 비교했는데 숫자가 같으면
    		...
    		for (int pos : moves) {
    			...
    			for (int i = 0; i < board.length; i++) {
    				...
    				if (board[i][pos - 1] != 0) {
    					// 일단 위에 숫자를 뽑으삼
    					int tmp = board[i][pos - 1];
    					// 그리고 이제 거기 있던 자리는 0으로 비워주삼
    					board[i][pos - 1] = 0;
    
    					// stack 비지 않았고 제일 위에 뽑으려는 숫자랑 tmp랑 같으면 숫자 팡해주기
    					if (!stack.isEmpty() && tmp ==stack.peek()) {
    						answer += 2;
    						stack.pop();
    					} else
...
- 도전2_실패(0525)
    ...
    public class Main {
    	...
    	public int solution(int[][] moves, int board[]) {
    		int answer = 0;
    		...
    		for (int pos : board) {
    			for (int i = 0; i < moves.length; i++) {
    				...
    				if (moves[i][pos - 1] != 0) {
    					stack.push(moves[i][pos - 1]);
    					moves[i][pos - 1] = 0;
    
    					if (!stack.isEmpty() && stack.peek() == moves[i][pos - 1]) {
    						answer += 2;
    					}
    					break;
    				}
    	...
    	public static void main(String[] args) {
    		...
    		int[] board = new int[m];
    		for (int i = 0; i < m; i++) {
    			board[m] = scan.nextInt();
    		}
    
    		System.out.println(main.solution(moves, board));
    	}
    }
    ```
